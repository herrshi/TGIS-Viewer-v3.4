...
import cn.seisys.TGISViewer.utils.ErrorMessageUtil;
import cn.seisys.TGISViewer.utils.LayerObjectUtil;
import cn.seisys.TGISViewer.utils.LocalizationUtil;
import cn.seisys.TGISViewer.utils.PortalBasemapAppender;

[Event(name="configLoaded", type="cn.seisys.TGISViewer.AppEvent")]
...
public class ConfigManager extends EventDispatcher
{
    ...
    private function configService_resultHandler(event:ResultEvent):void
    {
        try
        {
			...
			//================================================
			var socketEnable:Boolean = configXML.socketinfo.@enable == "true" ? true : false;
			if ( socketEnable )
			{
				var securitySocketIp:String = configXML.socketinfo.securitysocketip;
				var securitySocketPort:int = configXML.socketinfo.securitysocketport;
				var dataSocketIp:String = configXML.socketinfo.datasocketip;
				var dataSocketPort:int = configXML.socketinfo.datasocketport;
				configData.socketInfo = 
					{ 
						securitySocketIp: securitySocketIp, 
						securitySocketPort: securitySocketPort,
						dataSocketIp: dataSocketIp,
						dataSocketPort: dataSocketPort
					};
			}
			...
			//GIS服务器地址
			//================================================
			var gisServerIp:String = configXML.arcgisserverip;
			if ( gisServerIp )
			{
				configData.arcgisServerIP = gisServerIp;
			}
            ...
            if (geometryService)
            {
				var geometryServiceUrl:String = geometryService.@url[0] ? String(geometryService.@url[0]) : "";
				if ( gisServerIp )
				{
					geometryServiceUrl = geometryServiceUrl.replace( "{serverip}", gisServerIp );
				}
                configData.geometryService.url = geometryServiceUrl;
			...
			//导航条的样式
			var navigationStyleColor:Array = String( configXML.style.navigationcolors ).split( "," );
			for each ( colorStr in navigationStyleColor )
			{
				configData.styleNavigationColors.push( colorStr );
			}
			...
			var doubleClickZoomEnable:Boolean = configXML.map.@doubleclickzoomenabled[0] != false;
			var doubleClickZoomEnableAttr:Object =
				{
					id: "doubleClickZoom",
					doubleClickZoomEnable: doubleClickZoomEnable
				};
			mapAttrs.push(doubleClickZoomEnableAttr);
            ...
            else
            {
                ...
                {
                    configBasemaps.push(LayerObjectUtil.getLayerObject(maplayerList[i], i, false, bingKey, null, proxyUrl, gisServerIp));
                }
                ...
                {
                    configOpLayers.push(LayerObjectUtil.getLayerObject(opLayerList[i], i, true, bingKey, null, proxyUrl, gisServerIp));
                }
            ...
            if (!webMapItemID)
            {
                ...
                else
                {
					...
					worker.setSharedProperty( "configData", configData );
					trace( worker.getSharedProperty( "configData" ) );*/
					AppEvent.dispatch(AppEvent.CONFIG_LOADED, configData);
                }
            }
        }
    ...
    protected function basemapAppender_completeHandler(event:Event):void
    {
		...
		//设置worker共享数据
//		Worker.current.setSharedProperty( "configData", configData );
        AppEvent.dispatch(AppEvent.CONFIG_LOADED, configData);
    }
}
}
