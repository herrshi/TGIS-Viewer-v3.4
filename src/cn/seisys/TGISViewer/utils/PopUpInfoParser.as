...
{
    public static function parsePopUpInfo(popUpXML:XML):PopUpInfoWithButtons
    {
		...
		{
			popUpInfo.buttonArray = parsePopUpButtonArray( popUpXML.buttons[0] );
		}
		
		if ( popUpXML.idfieldname[0] )
		{
	...
	public static function parsePopUpButtonArray( buttonsXML:XML ):Array
	{
		var buttonArray:Array = [];
		for each ( var buttonXML:XML in buttonsXML.button )
		{
			var buttonObj:Object = 
				{
					label: String( buttonXML.@label ),
					action: String( buttonXML.@action ),
					type: String( buttonXML.@type ),
					idField: String( buttonXML.@idfield ),
					widgetName: String( buttonXML.@widgetname )
				};
