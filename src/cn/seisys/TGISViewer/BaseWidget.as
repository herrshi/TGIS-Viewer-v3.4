...
[Event(name="onChainRequest", type="cn.seisys.TGISViewer.AppEvent")]
...
public class BaseWidget extends Module implements IBaseWidget
{
    ...
    private function widgetChangeStateHandler(event:AppEvent):void
    {
		...
		{
			var reqLabel:String = data.label as String;
			
			if ( reqLabel == this.widgetTitle )
			{
				this.setState( reqState );
			}
		}
	...
	public function openWidget( widgetName:String ):void {
		AppEvent.dispatch( AppEvent.WIDGET_RUN, widgetName );
	}
	
	public function closeWidget( widgetName:String ):void {
